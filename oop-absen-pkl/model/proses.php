<?php 
session_start();
date_default_timezone_set('Asia/Jakarta'); // setting waktu default
include '../lib/db/dbconfig.php'; //mengkonfigurasi database

if (isset($_POST['login'])) { //method login untuk memproses login 
	$email = mysqli_real_escape_string($conn, $_POST['email']); //$email sebagai username
	$pwd = sha1(mysqli_escape_string($conn, $_POST['pwd']));//$password

	$sql = "SELECT * FROM user WHERE email_user='$email' AND pwd_user='$pwd'"; //$sql query untuk cek database
...
//
/**********************************************************/
elseif (isset($_GET['absen'])) { //query untuk mengambil data absensi
	if($_GET['absen']==1){
		$month = date("m");
		$day_tgl = date("d");
		$day = date("N");
